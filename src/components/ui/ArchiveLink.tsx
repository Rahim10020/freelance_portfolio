...
export default function ArchiveLink({ href, text, isExternal = false }: ArchiveLinkProps) {
    const content = (
        <span className="inline-flex items-center font-medium leading-tight text-[var(--c-text-primary)] group">
    ...
    if (isExternal) {
        return (
            <a
                ...
                className="inline-flex items-center text-base font-semibold leading-tight text-[var(--c-text-primary)] hover:text-[var(--text-accent)] focus-visible:text-[var(--text-accent)] transition-colors"
            >
                {content}
            </a>
        );
